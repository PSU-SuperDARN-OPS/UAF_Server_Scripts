...
####################################
DIR="/data/ros/fit/"
QNX_ROS_STOP="ssh root@mcm-ros /root/current_ros/stop.ros"
QNX_ROS_START="ssh root@mcm-ros /root/current_ros/start.ros"
QNX_REBOOT=""
LNX_ROS_STOP="stop.radar"
LNX_ROS_START="start.radar"
LNX_ROS_REBOOT=""
QNX_TEST_CMD="ssh root@mcm-ros  date -u"

EMAIL_RADAR="MCM"
EMAIL_FROM="MCM_radar_ops@superdarn.gi.alaska.edu"
...
if [ $diff -gt 600 ]
then
	  echo "Last fitacf write over 1 hour ago"
          emtime=`stat -c %Y ${EMAIL_ALERT_FILE}`
          errcond=$?
          if [ $errcond -eq 0 ]
          then
...
if [ $diff -gt 300 ]
then
	  echo "Last fitacf write over 5 minutes ago"
          emtime=`stat -c %Y ${EMAIL_ALERT_FILE}`
          errcond=$?
          if [ $errcond -eq 0 ]
          then
